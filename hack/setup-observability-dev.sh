...
# It assumes you have `kind` and `kubectl` installed and configured.

set -euo pipefail

# Configuration
: "${KIND_CLUSTER_NAME:=tekton-pipeline-old}"
...
setup_port_forwards() {
  ...
  kubectl port-forward -n monitoring svc/prometheus 8080:9090 > /dev/null 2>&1 &
  kubectl port-forward -n observability-system svc/jaeger 16687:16686 > /dev/null 2>&1 &
  kubectl port-forward -n tekton-pipelines svc/tekton-pipelines-controller 8081:9090 > /dev/null 2>&1 &
  kubectl port-forward -n tekton-pipelines svc/tekton-pipelines-webhook 8082:9090 > /dev/null 2>&1 &
  kubectl port-forward -n tekton-pipelines svc/tekton-events-controller 8083:9090 > /dev/null 2>&1 &
  kubectl port-forward -n tekton-pipelines-resolvers svc/tekton-pipelines-remote-resolvers 8084:9090 > /dev/null 2>&1 &
...
# Install Tekton Pipeline v1.9.0 from release
echo "Installing Tekton Pipeline v1.9.0..."
...
echo "  Prometheus: http://localhost:8080"
echo "  Jaeger: http://localhost:16687"
echo "  Controller Metrics: http://localhost:8081/metrics"
echo "  Webhook Metrics: http://localhost:8082/metrics"
echo "  Events Controller Metrics: http://localhost:8083/metrics"
echo "  Resolvers Metrics: http://localhost:8084/metrics"
echo ""
echo "Dump all metric names:"
echo "  # Controller metrics"
echo "  curl -s http://localhost:8081/metrics | grep '^# TYPE' | awk '{print \$3}' | sort"
echo ""
echo "  # Webhook metrics"
echo "  curl -s http://localhost:8082/metrics | grep '^# TYPE' | awk '{print \$3}' | sort"
echo ""
echo "  # Events controller metrics"
echo "  curl -s http://localhost:8083/metrics | grep '^# TYPE' | awk '{print \$3}' | sort"
echo ""
echo "  # Resolvers metrics"
echo "  curl -s http://localhost:8084/metrics | grep '^# TYPE' | awk '{print \$3}' | sort"
echo ""
echo "  # All Tekton-specific metrics across all components"
echo "  for port in 8081 8082 8083 8084; do curl -s http://localhost:\$port/metrics; done | grep '^# TYPE' | grep tekton | awk '{print \$3}' | sort -u"
echo ""
echo "  # All Knative metrics (workqueue, webhook, etc.)"
echo "  for port in 8081 8082 8083 8084; do curl -s http://localhost:\$port/metrics; done | grep '^# TYPE' | grep -E 'workqueue|webhook|rest_client' | awk '{print \$3}' | sort -u"
echo ""
...
echo "  curl -s http://localhost:8081/metrics | grep -E 'tekton_pipelines_controller_'"
echo "  curl -s http://localhost:8082/metrics | grep -E 'tekton_pipelines_webhook_'"
echo "  curl -s http://localhost:8083/metrics | grep -E 'tekton_events_'"
echo "  curl -s http://localhost:8084/metrics | grep -E 'tekton_resolution_'"
echo ""
